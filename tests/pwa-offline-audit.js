...
for(const s of ["saveKey='ruptura-save-v036'"])if(!main.includes(s))throw Error('missing '+s);
for(const s of ['ruptura-full-offline-v036','assets/player/','assets/enemies/','assets/bosses/','fases/cinza/level.js','fases/vazio/rooms.js','cacheAllIndividually','CACHE_STATUS'])if(!sw.includes(s))throw Error('missing '+s);
...
console.log(`PASS pwa-offline-audit v0.3.6: ${assetCount} assets + ${phaseCount} phase files precached`);
